...
## メモ
精子侵入点での拡散かどうかは明らかになっていない
（進行波ということになっている）
細胞質内は淡白が多いのでドロドロしている
IICRなどでは、均一なモデルを過程（実際の現象とは違う）

- 感度が高い実験系で見るとどうなるのか（今回の場合と同じになるか）
- 整形するときにどういったことが起きているのか（オリジナルデータを使うのが良いかどうか）（整形するときに何をregstrationにするか）
- 二種類の画像結果が一致するかどうかの比較。タンパクがどこに分布するかがdetectorの分布になる（タンパクの組み替えたい）

- 細胞がそもそも一様ではない？
- 遺伝子以外のものを操作して

- 拡散について：一部のエリアだけでCICRが起きて他では拡散の可能性もある
...
- 不均一性の根拠は？細胞の中にローカルなエリアが存在するかも？そこには、何らかのfunctionがある
細胞核の構造にはいくつかエリアが分かれることは知られている。（waveのメカニズムのとは違う話）

1. dye（カルシウムがくっつくと色がつく）（細胞の中に化学物質を入れていて、カルシウムが近くにあるとくっつきひかる。細胞内にケミカルが一様に入っているはず。）でやっている場合。撮影する直前に色素を入れる必要がある
2. 細胞のDenaに人工的なタンパク質を作るようにして、そのタンパク質はカルシウムが近くに来ると光る。あるタンパク質が細胞内にあり、カルシウムが近くにあると光るイメージ。実験が簡単になった。1の場合よりも蛍光が10倍ぐらい強い。  
タンパク質で見たときと、dye（ケミカル）で撮ったダイナミクスが同じかわからない。一様に両方とも広がるなら、反応速度の違いぐらいしかあらわれない。そのタンパク質がどう分布するかと（化学物質の方が安定的に分布する（テキストランに依存））。この2つの差を見ると何がわかる？
...
## 相談したいことについて
1. CICRとIICRによってカルシウム波の変化にどのような違いが考えられるか  

2. カルシウムを放出する小器官の非均一性に関する議論  

3. カルシウム蛍光の最大値が場所によって違うのが、何らかの因子が原因ではないかと考えた背景  
- 受精前の細胞に極性がない。
受精したからこそ、不均一性が生まれて、それがカルシウムのピークの差として現れる？
- 受精前から、細胞には不均一性があって、精子が入りそうな場所に細胞がいっぱいある？
- 細胞内のカルシウム強度に不均一性が見れればいい。もし均一ならば、時間をずらして相関を見ればわかるはず。
...
- Waveが単純に流れているならば、右から左への流れが期待されるが、カルシウム濃度が減っていく場合のメカニズムが明らかになっておらず、その影響が無視できない
- どのくらい時間をずらしたら因果が最も強くなるかがわかれば、隣のピクセルへの伝達速度がわかる？潜在因子があれば、その因子の伝わっていく速度が現れる？？？
- オプティカルフロー（瞬間的なアプローチ）と全体的なタイムポイントでのアプローチ
- 二つのフェーズで分けて因果推論できるようにする？
- 局所波と大域波波は同じメカニズムではないか？
